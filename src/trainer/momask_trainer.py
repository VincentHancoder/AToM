...
class MomaskDPOTrainer(Trainer):
    r"""
    ...
    def __init__(
        ...
        super().__init__(
            ...
            tokenizer=tokenizer,
            model_init=model_init, # None
            compute_metrics=compute_metrics, # None
            callbacks=callbacks, 
            optimizers=optimizers, # None
            preprocess_logits_for_metrics=preprocess_logits_for_metrics,  # None
        )
    ...
    def tokenize_row(self, feature, model: Union[PreTrainedModel, nn.Module] = None) -> Dict:
        ...
        chosen_len = feature["chosen_len"]
        rejected = feature["rejected"]
        rejected_len = feature["rejected_len"]
        
        label_smoothing = feature["label_smoothing"]
        ...
        if not self.is_encoder_decoder:
            ...
            batch["chosen_labels"] = chosen
            batch["chosen_len"] = chosen_len
            batch["rejected_labels"] = rejected
            batch["rejected_len"] = rejected_len
            batch["prompts"] = prompt

            batch["label_smoothing"] = label_smoothing
        # import pdb; pdb.set_trace()
        return batch
    ...
    def concatenated_inputs(
        ...
        # import ipdb
        # ipdb.set_trace()
        concatenated_batch["concatenated_prompts"] = batch["prompt"] * 2
    ...
    def dpo_loss(
        ...
        label_smoothing: torch.tensor = None
        ...
        if self.loss_type == "sigmoid":
            if label_smoothing is not None:
                # import pdb; pdb.set_trace()
                losses = (
                    # logits size is 2*bs and label_smoothing too
    ...
    def concatenated_forward(
        ...
        m_lens = concatenated_batch["concatenated_len"][:, 0]

        outputs = model(
            concatenated_batch["concatenated_labels"],
            concatenated_batch["concatenated_prompts"],
            m_lens
        )
        ...
        all_logits = outputs[-1].permute(0, 2, 1)
    ...
    def get_batch_loss_metrics(
        ...
        losses, chosen_rewards, rejected_rewards = self.dpo_loss(
            ...
            reference_chosen_logps,
            reference_rejected_logps,
            label_smoothing=batch['label_smoothing'] if self.use_label_smoothing else None
        )
