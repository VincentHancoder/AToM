...
def render(npydata,
    ...
    # frameidx =  [0, 40, 84, 100]
    # # frameidx[-2] -= 5

    # delta_x = np.array([0.3, 0.0, 0.0])
    # # # delta_x = np.array([0.025, 0.0, 0.0])
    # # delta_y = np.array([0.0, 0.4, 0.0])
    # # # delta_y = np.array([0.0, 0.025, 0.0])

    # for i, idx in enumerate(frameidx):
    #     data.data[idx, :, :] += i * delta_x  # x轴平移

    # data.data[frameidx[4], :, :] += 2.5 * delta_x  # x轴平移
    
    # for i, idx in enumerate(frameidx[5:]):
    #     data.data[idx, :, :] -= (i+1) * delta_x  # x轴平移

    # # for i, idx in enumerate(frameidx[5:]):
    # #     data.data[idx, :, :] += -i * delta_x + delta_x  # x轴平移

    # for i, idx in enumerate(frameidx[4:]):
    #     data.data[idx, :, :] += delta_y  # x轴平移
    ...
    # camera = Camera(first_root=data.get_root(80), mode=mode, is_mesh=is_mesh)
    camera = Camera(first_root=data.get_root(0), mode=mode, is_mesh=is_mesh)


    
    # frameidx = [20, 29, 60, 78, 124, 135, 150]
