...
def render_cli() -> None:
    ...
    init = True
    for path in paths:
        ### avoid trying to render _token.npy feats.npy files
        if not (path.endswith("out.npy") or path.endswith("mesh.npy")):
        # if not path.endswith("mesh.npy") or (not "finetune" in path):
        # if not path.endswith("mesh.npy"):
            continue
