...
from mGPT.utils.load_checkpoint import load_pretrained_test_peft,  load_pretrained_vae
...
def main():
    ...
    if cfg.TEST.CHECKPOINTS:
        if cfg.TEST.CHECKPOINTS.endswith(".pt") or cfg.PEFT:
    ...
    if text:
        # for t in range(10):
        for t in range(1):
            for b in tqdm(range((len(text)-1+batch_size) // batch_size)):
                ...
                ### generete five samples for one prompt
                ...
                for i in range(len(joints)):
                    ...
                    np.save(os.path.join(output_dir, f'{id}/{t}_out.npy'), xyz)
                    # np.save(os.path.join(output_dir, f'{id}_in.npy'), xyz_in)
                    np.save(os.path.join(output_dir, f"{id}/{t}_out_feats.npy"), feature)

                    with open(os.path.join(output_dir, f'{id}/prompt.txt'), 'w') as f:
                        f.write(text_batch[i])

                    ## calculate motion tokens
                    feature = torch.tensor(feature, device=model.device)
                    feature = model.datamodule.normalize(feature)
                    motion_token, _ = model.vae.encode(feature[None])
                    motion_token = motion_token.detach().cpu().numpy()
                    np.save(os.path.join(output_dir, f"{id}/{t}_out_feats_token.npy"), motion_token)
